...
export default function Marquee() {
  ...
  return (
    <section className="py-12 bg-white dark:bg-brand-primary border-y border-brand-light-slate dark:border-slate-800 overflow-hidden">
      ...
      <div className="relative flex w-full overflow-hidden">
        {/* Left/Right Fades */}
        <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-white dark:from-brand-primary to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-white dark:from-brand-primary to-transparent z-10 pointer-events-none" />
        ...
        >
          {duplicatedTech.map((tech, index) => (
            <div 
              key={index} 
              className="flex items-center gap-3 px-8 py-4 mx-4 bg-brand-light-bg dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm"
            >
              <div className="text-brand-accent">{tech.icon}</div>
              <span className="font-semibold text-brand-primary dark:text-white text-lg">{tech.name}</span>
            </div>
          ))}
        </motion.div>
      </div>
